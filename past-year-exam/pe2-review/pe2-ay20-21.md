...
So, for this question, during every iteration, the possible characters we can choose from 0 to `len-k` in the current `src` string! And once we have added a character to our `sub` string, we move our `src` string to the remaining string behind that character.
