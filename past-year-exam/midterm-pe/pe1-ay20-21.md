...
First, let's clarify our goal. Remember that the reason we can update `n` to `curr` is because that we only need to find the factors of `curr`. So, after every iteration, we have already eliminated all the factors that are smaller `sqrt(curr)`. This ensures that when we reach the termination of the loop, the remaing `curr` must be the last remaining prime one.
